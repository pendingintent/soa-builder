...
def test_create_and_normalize_flow():
    ...
    soa_id = r.json()['id']
    # add visit
    rv = client.post(f'/soa/{soa_id}/visits', json={'name': 'C1D1', 'raw_header': 'Cycle 1 Day 1 (C1D1)'})
    ...
    # set cell
    rc = client.post(f'/soa/{soa_id}/cells', json={'visit_id': visit_id, 'activity_id': activity_id, 'status': 'X'})
