...
def _init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS soa (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, created_at TEXT)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS visit (id INTEGER PRIMARY KEY AUTOINCREMENT, soa_id INTEGER, name TEXT, raw_header TEXT, order_index INTEGER)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS activity (id INTEGER PRIMARY KEY AUTOINCREMENT, soa_id INTEGER, name TEXT, order_index INTEGER)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS cell (id INTEGER PRIMARY KEY AUTOINCREMENT, soa_id INTEGER, visit_id INTEGER, activity_id INTEGER, status TEXT)""")
...
def _soa_exists(soa_id: int) -> bool:
    conn = _connect(); cur = conn.cursor(); cur.execute("SELECT 1 FROM soa WHERE id=?", (soa_id,)); row = cur.fetchone(); conn.close(); return row is not None

def _fetch_matrix(soa_id: int):
    conn = _connect(); cur = conn.cursor()
    cur.execute("SELECT id,name,raw_header,order_index FROM visit WHERE soa_id=? ORDER BY order_index", (soa_id,))
    visits = [dict(id=r[0], name=r[1], raw_header=r[2], order_index=r[3]) for r in cur.fetchall()]
    cur.execute("SELECT id,name,order_index FROM activity WHERE soa_id=? ORDER BY order_index", (soa_id,))
    activities = [dict(id=r[0], name=r[1], order_index=r[2]) for r in cur.fetchall()]
    cur.execute("SELECT visit_id, activity_id, status FROM cell WHERE soa_id=?", (soa_id,))
...
def _generate_wide_csv(soa_id: int) -> str:
    ...
    if not visits or not activities:
        raise ValueError("Cannot generate CSV: need at least one visit and one activity")
    ...
    for a in activities:
        row = [a['name']]
        for v in visits:
            match = next((c['status'] for c in cells if c['visit_id']==v['id'] and c['activity_id']==a['id']), '')
            row.append(match)
        matrix.append(row)
...
def create_soa(payload: SOACreate):
    conn = _connect(); cur = conn.cursor()
    cur.execute("INSERT INTO soa (name, created_at) VALUES (?, ?)", (payload.name, datetime.now(timezone.utc).isoformat()))
...
def get_soa(soa_id: int):
    if not _soa_exists(soa_id): raise HTTPException(404, 'SOA not found')
    visits, activities, cells = _fetch_matrix(soa_id)
...
def add_visit(soa_id: int, payload: VisitCreate):
    if not _soa_exists(soa_id): raise HTTPException(404, 'SOA not found')
    conn = _connect(); cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM visit WHERE soa_id=?", (soa_id,)); order_index = cur.fetchone()[0] + 1
    cur.execute("INSERT INTO visit (soa_id,name,raw_header,order_index) VALUES (?,?,?,?)", (soa_id, payload.name, payload.raw_header or payload.name, order_index))
    vid = cur.lastrowid; conn.commit(); conn.close()
...
def add_activity(soa_id: int, payload: ActivityCreate):
    if not _soa_exists(soa_id): raise HTTPException(404, 'SOA not found')
    conn = _connect(); cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM activity WHERE soa_id=?", (soa_id,)); order_index = cur.fetchone()[0] + 1
    cur.execute("INSERT INTO activity (soa_id,name,order_index) VALUES (?,?,?)", (soa_id, payload.name, order_index))
...
def set_cell(soa_id: int, payload: CellCreate):
    ...
    cur.execute("SELECT id FROM cell WHERE soa_id=? AND visit_id=? AND activity_id=?", (soa_id, payload.visit_id, payload.activity_id))
    row = cur.fetchone()
    if row:
        cur.execute("UPDATE cell SET status=? WHERE id=?", (payload.status, row[0]))
        cid = row[0]
    else:
        cur.execute("INSERT INTO cell (soa_id, visit_id, activity_id, status) VALUES (?,?,?,?)", (soa_id, payload.visit_id, payload.activity_id, payload.status))
        cid = cur.lastrowid
    conn.commit(); conn.close()
...
def get_normalized(soa_id: int):
    ...
    summary = normalize_soa(csv_path, out_dir, sqlite_path=os.path.join(out_dir, 'soa.db'))
...
def _reindex(table: str, soa_id: int):
    conn = _connect(); cur = conn.cursor()
    cur.execute(f"SELECT id FROM {table} WHERE soa_id=? ORDER BY order_index", (soa_id,))
...
def delete_activity(soa_id: int, activity_id: int):
    ...
    cur.execute("DELETE FROM cell WHERE soa_id=? AND activity_id=?", (soa_id, activity_id))
...
def ui_index(request: Request):
    conn = _connect(); cur = conn.cursor(); cur.execute("SELECT id,name,created_at FROM soa ORDER BY id DESC"); rows = cur.fetchall(); conn.close()
    return templates.TemplateResponse('index.html', {"request": request, "soas": [{"id": r[0], "name": r[1], "created_at": r[2]} for r in rows]})

@app.post('/ui/soa/create', response_class=HTMLResponse)
def ui_create_soa(request: Request, name: str = Form(...)):
    conn = _connect(); cur = conn.cursor(); cur.execute("INSERT INTO soa (name, created_at) VALUES (?,?)", (name, datetime.now(timezone.utc).isoformat())); sid = cur.lastrowid; conn.commit(); conn.close()
...
def ui_edit(request: Request, soa_id: int):
    ...
    return templates.TemplateResponse('edit.html', {"request": request, "soa_id": soa_id, "visits": visits, "activities": activities, "cell_map": cell_map})

@app.post('/ui/soa/{soa_id}/add_visit', response_class=HTMLResponse)
def ui_add_visit(request: Request, soa_id: int, name: str = Form(...), raw_header: str = Form('')):
...
def ui_set_cell(request: Request, soa_id: int, visit_id: int = Form(...), activity_id: int = Form(...), status: str = Form('X')):
    set_cell(soa_id, CellCreate(visit_id=visit_id, activity_id=activity_id, status=status))
