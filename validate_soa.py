...
def validate_imaging(visits: pd.DataFrame, va: pd.DataFrame, imaging_ids: list[int], tolerance_days: int):
    if not imaging_ids:
    ...
    imaging_visits = imaging_va.merge(visits, left_on='visit_id', right_on='visit_id', how='left')
    imaging_visits = imaging_visits.sort_values('sequence_index')
    errors = []
    if len(imaging_visits) < EXPECTED_IMAGING_COUNT:
        errors.append(f"Expected >= {EXPECTED_IMAGING_COUNT} required imaging visits; found {len(imaging_visits)}.")
    # Compute intervals
    ...
    for i in range(1, len(nominal_days)):
        ...
        if abs(delta - expected_days) > tolerance_days:
            ...
            errors.append(f"Imaging interval between {vnames[0]} and {vnames[1]} is ~{delta:.1f}d (expected ~{expected_days}d ±{tolerance_days}d).")
    return len(errors) == 0, errors


def main():
    ap = argparse.ArgumentParser(description="Validate normalized SoA schedule")
    ap.add_argument('--dir', required=True, help='Directory containing normalized CSV outputs')
    ap.add_argument('--tolerance-days', type=int, default=DEFAULT_TOLERANCE_DAYS, help='Tolerance for imaging interval deviations')
    args = ap.parse_args()

    visits, activities, va = load_normalized(args.dir)
    imaging_ids = find_imaging_activity_ids(activities)
    imaging_ok, imaging_errors = validate_imaging(visits, va, imaging_ids, args.tolerance_days)
